...
@Component({
  selector: 'ngx-Map-1-simulation',
  styleUrls: ['./Map-1-simulation.component.scss'],
 template: `
...
export class Map1simulationComponent implements OnInit,OnChanges {
  ...
  changeDate(event){
//  this.msgFromChild1= [];
    //this.msgFromChild1[0]=this.currentMsgToParent;
    

    if(this.currentMsgToParent == 'null'){
      this.tiimes.emit(this.selectedDateMap);
    }else{
  ...
  ngOnChanges(){
    ...
    this.selectedDateMap = this.Dates[1]; 
    this.changeDate(this.Layers[1])
