...
export class HSTableComponent implements OnInit {
  ...
  export() {
    this.exportService.exportExcel(this.excel_, 'Soil_humidity');
  }
  ...
  constructor(private exportService: ExportService,
   ...
   this.mesureServes.getMesures().subscribe((item) => {
       this.mesures = item;
    item.map(e => {
     if(e.mesure == "soil_humidity"){
       
         e.data.map(a => {
           
           a.value.map(b => {   this.valueTime.push(
             {id_exploitation: e.id_exploitation,mesure :  '<i class="fas fa-hand-holding-water"></i>',
