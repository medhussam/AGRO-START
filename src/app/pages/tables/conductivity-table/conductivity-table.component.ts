...
@Component({
  selector: 'ngx-conductivity-table',
  templateUrl: './conductivity-table.component.html',
  styleUrls: ['./conductivity-table.component.scss']
})
export class ConductivityTableComponent implements OnInit {
  mesures: any;
  temperature: any[] = [];
  valueTime: any[] = [];
  ...
  export() {
    this.exportService.exportExcel(this.excel_, 'Conductivity');
  }
  ...
  constructor(private exportService: ExportService,
   ...
   this.mesureServes.getMesures().subscribe((item) => { 
       this.mesures = item;
    item.map(e => {
     if(e.mesure == "soil_conductivity"){
       
         e.data.map(a => {
           
           a.value.map(b => {   this.valueTime.push(
             {id_exploitation: e.id_exploitation,mesure :  '<i class="fas fa-bolt"></i>',
