...
@Component({
  selector: 'ngx-hummidity-table',
  templateUrl: './hummidity-table.component.html',
  styleUrls: ['./hummidity-table.component.scss']
})
export class HummidityTableComponent implements OnInit {
  mesures: any;
  temperature: any[] = [];
  valueTime: any[] = [];
  ...
  export() {
    this.exportService.exportExcel(this.excel_, 'Humidity');
  }
     ...
     if(e.mesure == "humidity"){
         ...
         e.data.map(a => {
           ...
           a.value.map(b => {   this.valueTime.push(
             {id_exploitation: e.id_exploitation,mesure :  '<i class="fas fa-tint"></i>',
